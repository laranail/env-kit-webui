...
it('ipAllowed: open with no allowlist, matches CIDR, rejects others', function () {
    config(['laranail.env-kit-webui.access.allowed_ips' => []]);
    expect(PanelAccess::ipAllowed(reqFrom('203.0.113.9')))->toBeTrue();

    config(['laranail.env-kit-webui.access.allowed_ips' => ['10.0.0.0/8', '::1']]);
...
it('withinSchedule: open when nothing is configured', function () {
    config(['laranail.env-kit-webui.access.schedule' => []]);
...
it('withinSchedule: honours an allowed-day set (names + ISO numbers)', function () {
    ...
    config(['laranail.env-kit-webui.access.schedule' => ['timezone' => 'UTC', 'days' => [$today]]]);
    expect(PanelAccess::withinSchedule())->toBeTrue();

    config(['laranail.env-kit-webui.access.schedule' => ['timezone' => 'UTC', 'days' => [$other]]]);
    expect(PanelAccess::withinSchedule())->toBeFalse();

    config(['laranail.env-kit-webui.access.schedule' => ['timezone' => 'UTC', 'days' => [$now->dayOfWeekIso]]]);
    expect(PanelAccess::withinSchedule())->toBeTrue();
});
...
it('withinSchedule: same-day and overnight daily windows', function () {
    ...
    config(['laranail.env-kit-webui.access.schedule' => ['timezone' => 'UTC', 'start' => '22:00', 'end' => '06:00']]);
...
it('withinSchedule: honours an absolute from/until range', function () {
    config(['laranail.env-kit-webui.access.schedule' => ['timezone' => 'UTC', 'from' => '2026-06-01', 'until' => '2026-06-15']]);
